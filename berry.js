(function () {
    ...
    function ApiPWA() {
      ...
      this.account = function (u, join) {
        if (join) {
          if (Defined.use_api == 'pwa' && (u.indexOf('http://') > -1 || u.indexOf('https://') > -1)) return u;
          if (Defined.use_api == 'http' && u.indexOf(Defined.localhost) == -1) return u;
        }
    ...
    function ApiHttp() {
      ...
      this.account = function (u) {
        if (Defined.use_api == 'pwa' && (u.indexOf('http://') > -1 || u.indexOf('https://') > -1)) return u;
        if (Defined.use_api == 'http' && u.indexOf(Defined.localhost) == -1 && window.location.hostname !== 'localhost') return u;
        var unic_id = Lampa.Storage.get('sisi_unic_id', '');
        var email = Lampa.Storage.get('account', {}).email;
        if (u.indexOf('box_mac=') == -1) u = Lampa.Utils.addUrlComponent(u, 'box_mac=' + unic_id);else u = u.replace(/box_mac=[^&]+/, 'box_mac=' + unic_id);
      ...
      function add() {
        ...
        button.on('hover:enter', function () {
          Api.menu(function (data) {
            ...
            if (Defined.use_api == 'http' || Lampa.Platform.is('android')) {
              items.push({
                title: 'Все'
              });
            }
